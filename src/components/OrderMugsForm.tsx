  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (exported) {
        ...
        if (exported.mockPreview) {
          if (exported.mockPreview.size > PREVIEW_MAX_SIZE_MB * 1024 * 1024) {
            setFormError(`Mock-превью слишком большое. Максимум ${PREVIEW_MAX_SIZE_MB} МБ.`);
            setIsSending(false);
            return;
          }
          formData.set('mockPreview', exported.mockPreview, exported.mockPreview.name);
        }
